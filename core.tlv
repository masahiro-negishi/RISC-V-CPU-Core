...
\SV
   ...
   //m4_asm(ADDI, x0, x0, 1) // Confirm that non-zero value cannot be written to x0
   //m4_asm_end()
...
\TLV
   ...
   $rd_valid = $rd == 5'b0 ? 1'b0 : // Cannot write to x0
               $is_i_instr || $is_r_instr || $is_u_instr || $is_j_instr;
   ...
   $is_load = $opcode== 7'b0000011; //$is_store = $is_s_instr
   ...
   $result[31:0] = $is_andi ? $src1_value & $imm :
                   ...
                   $is_auipc ? $pc + $imm :
                   $is_jal ? $pc + 32'd4 : // Store current PC + 4 in R[rd]
                   $is_jalr ? $pc + 32'd4 :
